...
def main(args):
    ...
    # 如果存在预训练权重则载入
    if args.version == 2:
        model = MobileNetV2(in_channel=1, num_classes=args.num_classes).to(device)
    elif args.version == 3:
        model = mobilenet_v3_large(in_channel=1, num_classes=args.num_classes).to(device)
    else:
    ...
    plot_accuracy(args.epochs, total_accuracy_train, total_accuracy_val, "MobileNet_CK+")
